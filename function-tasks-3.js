...
console.group("12. Sukurkite funkciją, kuri grąžina balsių kiekį žodyje");
{
  function getNumberOfVowels(str) {
    ...
    for (let i = 0; i < balses.length; i++) {
      numb.push(str.split(balses[i]).length - 1);
      const reducer = (prevValue, curValue) => prevValue + curValue;
      sum = numb.reduce(reducer);
    }
    return sum;
  }
  console.log("---");
...
);
{
  function filterLetters(str, lettersToRemove) {
    console.log(str, lettersToRemove);
    return str, lettersToRemove;
  }
  console.log("---");
...
);
{
  function capitalize(str) {
    ...
    let test1 = [];
    for (let i = 0; i < str.length; i++) {
      test1.push(str[i][0].toLocaleUpperCase() + str[i].slice(1));
    }
    return test1;
  }
  console.log("---");
...
);
{
  function strReverse(str) {
    ...
    let test1 = [];
    for (let i = 0; i < str.length; i++) {
      test1.push(str[i].split("").reverse().join(""));
    }
    console.log(test1);
  }
  console.log("---");
